...
let anwserStatus = document.getElementById("anwserStatus");
let gameStatus = document.getElementById("gameStatus");
let flashcards = document.getElementById('flashcards');

let askedQuestions = [];
const ww1Questions = carddata.filter(item => item.category === "ww1");
...
function startGame(category) {
    scorestatus.textContent = `Score: ${game.score}`;
    gameStatus.textContent = 'Game in Progress!'
    anwserStatus.textContent = 'Please Select an Option'
    game.state = "playing";
    game.cat = category;
    newQuestion();
    let myCard = createCard();
    flashcards.appendChild(myCard);
}


function newQuestion() {
    let availableQuestions = carddata.filter(q => !askedQuestions.includes(q.question)&& q.category === game.cat);
    if (availableQuestions.length === 0) {
        gameStatus.textContent = `All Questions Awnsered! You got ${game.correctChoices} correct and ${game.incorrectChoices} wrong!`
        game.state = "over"
        return;
    }

    let data = availableQuestions[Math.floor(Math.random() * availableQuestions.length)];
    game.question = data.question;
    game.answer = data.answer;
    game.choices = data.choices;

    askedQuestions.push(game.question);
}
...
function createCard() {
    if (game.state === "playing") {
        ...
        cardTitle.textContent = game.question;
        ...
        game.choices.forEach(choice => {
            ...
            button.disabled = game.state === "over";
            button.onclick = () => checkAnswer(choice, game.answer);
...
function checkAnswer(choice, correctAnswer) {
    if (game.state === "playing"){
        if (choice === correctAnswer) {
            anwserStatus.textContent = 'Correct'
            game.score += 1;
            game.correctChoices += 1;
        } else {
            game.score -= 1;
            game.incorrectChoices += 1;
            anwserStatus.textContent = `Incorrect! Anwser was "${game.answer}"`;
        }
        contGame();
    }
}
